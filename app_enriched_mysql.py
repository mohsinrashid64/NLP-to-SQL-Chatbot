...
- Uses LLM to analyze database and generate rich descriptions
- Stores descriptions in JSON file for reuse
- Creates embeddings from enriched descriptions for better matching
- Uses local sentence-transformers for embeddings (no API needed)
"""
...
EMBEDDING_DEPLOYMENT = "text-embedding-ada-002"  # <-- UPDATE THIS if different in Azure
...
def setup_chromadb(recreate=False):
    ...
    if recreate:
        ...
        try:
            chroma_client.delete_collection("enriched_schemas")
        except:
            pass
...
def load_existing_embeddings():
    ...
    try:
        chroma_client = chromadb.PersistentClient(path=CHROMA_DB_PATH)
        collection = chroma_client.get_collection("enriched_schemas")
...
def create_embeddings_from_descriptions(progress=gr.Progress()):
    """Create embeddings from enriched descriptions (recreates existing)"""
    ...
    try:
        ...
        progress(0, desc="Creating embeddings...")
...
"""
        ...
        return f"✅ Created embeddings for {total} tables"
...
def find_relevant_tables(question: str, top_k: int = 5) -> list:
    """Find relevant tables using enriched embeddings"""
    global collection
...
def extract_sql(response: str) -> str:
    ...
    # Validate it's actually SQL, not an explanation
    if not found_sql:
        return ""

    # Check if the response is mostly an explanation (not valid SQL)
    invalid_phrases = [
    ...
    # Basic validation - SQL should start with a valid keyword
...
def ask_question(question: str, host: str, port: str, user: str, password: str, database: str):
    ...
    try:
        ...
        # Get table names and their info
...
- For counting or reporting, use COUNT() and GROUP BY as needed"""
        ...
        try:
            ...
            # Convert datetime.datetime(...) to string format for parsing
            clean_result = re.sub(
...
# Gradio UI
with gr.Blocks(title="NLP to SQL - Enriched Schema", theme=gr.themes.Soft()) as demo:
    gr.Markdown("# 🧠 NLP to SQL with Enriched Schema Descriptions")
    gr.Markdown("Uses LLM to analyze database and create rich descriptions for better query understanding")
    ...
    with gr.Tab("2️⃣ Generate Schema Descriptions"):
        gr.Markdown("""### Analyze Database with LLM
        This will analyze each table using GPT-4 and generate rich descriptions including:
    ...
    with gr.Tab("3️⃣ Create Embeddings"):
        gr.Markdown("### Create Embeddings from Enriched Descriptions")
        gr.Markdown("Embeddings are saved to disk and persist across restarts. Uses local model (no API needed).")
    ...
    with gr.Tab("📝 Edit Schema"):
        ...
        You can edit the schema file to add custom business terms, fix descriptions, or add example questions.
        """)
...
if __name__ == "__main__":
    # Try to load existing schema and embeddings on startup
    load_schema_descriptions()
    exists, count = load_existing_embeddings()
    if exists:
